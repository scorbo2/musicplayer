...
/**
 * Allows the application to enforce a single running instance.
 ...
 * <p>
 *     <b>Note:</b> This class will be moved to the swing-extras library
 *     in the next release of swing-extras, so that other applications
 *     can make use of it. For now, it can live here.
 * </p>
 *
...
public class SingleInstanceManager {
    ...
    public boolean tryAcquireLock(ArgsListener listener, int port) {
        ...
        try {
            ...
            log.info("MusicPlayer is already running (port " + port + " is in use).");
            return false;
        }
    }
    ...
    /**
     * Handle incoming file paths from new instances.
     */
        ...
        new Thread(() -> {
            try (BufferedReader in = new BufferedReader(
                ...
                if (argsListener != null) {
                    ...
                    javax.swing.SwingUtilities.invokeLater(() -> argsListener.argsReceived(filePaths));
                }
    ...
    public void release() {
        try {
            if (serverSocket != null && !serverSocket.isClosed()) {
                serverSocket.close();
                log.info("MusicPlayer single instance manager on port " + port + " has been shut down.");
            }
        } catch (IOException e) {
